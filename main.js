...
$(document).ready(function() {
    // --- KONFIGURASI API ---
    // GANTI DENGAN KUNCI API GOOGLE BOOKS ANDA YANG ASLI
    ...
    function createBookCard(book) {
        ...
        const coverUrl = info.imageLinks ? info.imageLinks.thumbnail : 'https://via.placeholder.com/150x220?text=No+Cover';
        ...
        const gradient = averageRating >= 4.0 ? 'from-green-400 to-blue-500' : 'from-gray-300 to-gray-400';
        ...
        for (let i = startPage; i <= endPage; i++) {
             paginationHTML += `
                <button onclick="changePage(${i})" 
                        class="px-4 py-2 mx-1 rounded-lg font-semibold transition ${i === currentPage ? 'bg-blue-600 text-white shadow-md' : 'bg-white text-gray-700 hover:bg-blue-100'}">
                    ${i + 1}
                </button>
            `;
        }
    ...
    async function renderBooks(pageIndex = currentPage) {
        if (GOOGLE_BOOKS_API_KEY === 'YOUR_GOOGLE_BOOKS_API_KEY') {
             $('#books-grid').html('<p class="col-span-full text-red-500 font-bold text-center py-10">⚠️ Harap masukkan API Key Google Books Anda di file main.js!</p>');
        ...
        booksGrid.innerHTML = `
            <div class="col-span-full text-center py-10">
                <p class="text-blue-600 font-semibold text-lg animate-pulse">Memuat koleksi halaman ${currentPage + 1}...</p>
            </div>
        `;
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(`Gagal memuat: Status ${response.status}. Cek API Key.`);
            }
    ...
    $('#category-buttons').on('click', '.category-btn', function() {
        ...
        $('.category-btn').removeClass('bg-blue-600 text-white shadow-lg').addClass('bg-gray-100 text-gray-700');
        $(this).removeClass('bg-gray-100 text-gray-700').addClass('bg-blue-600 text-white shadow-lg');
